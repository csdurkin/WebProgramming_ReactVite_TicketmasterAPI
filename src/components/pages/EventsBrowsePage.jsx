...
//FORM: HTML AND HANDLING 
    ...
    // EventSearch: Function declaration, props: handleSearch function
    function EventSearch({ handleSearch }) {
      ...
      const validateSearchInput = () => {
          ...
          if (!eventSearchTerm || eventSearchTerm.trim().length < 3) {
      ...
      const processSearchSubmission = (event) => {
          ...
          handleSearch(eventSearchTerm.trim());
      ...
      return (
          ...
          <form onSubmit={processSearchSubmission} className="search-bar">
              ...
              <div>
                  ...
                  <label htmlFor="eventSearchTerm">Search Events:</label>
                  <input
                      type="text"
                      id="eventSearchTerm"
                      name="attractionSearchTerm"
                      value={eventSearchTerm}
                      // onChange: Updates the eventSearchTerm state with the current value of the input field.
...
//EVENTS BROWSE PAGE: Data Fetching, Search, and Pagination
    ...
    const EventsBrowsePage = () => {
      ...
      // STATE MANAGEMENT 
          ...
          //Create state variable (eventsData) and the setter function (setEventsData)
          ...
          useEffect(() => {
            ...
            async function fetchEvents() {
              ...
              try {
                ...
                //Use axios to pull form the Ticketmaster API the data for the events
                ...
                //Check if there are events for the page; if not, redirect to 404
                ...
                if (!data._embedded?.events?.length) {
                  alert('No matching results found. Showing all events.');
                  setSearchTerm('');
                  navigate('/events/page/1');
                  return;
                }

                //Set the events data and pagination info in state
                ...
                console.error('An error occurred when fetching events:', e);
                navigate('/404');
              }
            }
            ...
            /* Dependency Array, definitions for this case: 
                  - page: current page of events; when this changes, the effect is triggered to fetch new data
                  - searchTerm: current search term for events; triggers new data fetch if updated
      ...
      //LOADING STATE AND CONDITIONAL RENDERING
          ...
          //So, loading is initially set to true, but the fetchEvents() sets it to false once it has pulled the data  
          if (loading) {
            return <h2>Loading...</h2>;
          }

          //If no events data pulled
      ...
      //DISPLAY UPDATE
      //RETURN FOR THE HIGHEST LEVEL FUNCTION: EventsBrowsePage
      ...
      return (
            ...
            {eventsData.map((event) => (
              ...
              <Grid item xs={12} sm={6} md={4} lg={3} key={event.id}>
                ...
                <Card sx={{ maxWidth: 250, height: 'auto', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  ...
                  <CardMedia
                    component="img"
                    image={event.images?.[0]?.url || noImage}
                    alt={event.name || 'Event Image'}
                    sx={{ height: 180, objectFit: 'cover' }}
                  />

                  <CardHeader
                    title={event.name || 'No Event Name Available'}
        ...
        */}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '20px' }}>
            {parseInt(page) > 1 && (
              <button onClick={() => navigate(`/events/page/${parseInt(page) - 1}`)}>Previous</button>
            )}
            {parseInt(page) < totalPages && (
              <button onClick={() => navigate(`/events/page/${parseInt(page) + 1}`)}>Next</button>
            )}
          </div>
        </div>
      );

      //CLOSE OF: EventsBrowsePage
    };
